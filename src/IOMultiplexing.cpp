...
void IOMultiplexing::EventLoop(std::vector<Server &> &servers)
{
    ...
    event.data.fd = io._fdmax; // Il file descriptor da monitorare
    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, io._fdmax, &event) == -1) 
	{
    ...
    while (1) {
        epoll_event events[io._fdmax + 1];
        int ready_fds = epoll_wait(epoll_fd, events, io._fdmax + 1, 3000); // Timeout di 3 secondi
        if (ready_fds == -1) {
