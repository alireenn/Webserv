                    ...
                    for (size_t k = 0; k < ClientsRequest.size(); k++)
                    {
                        if (fd == ClientsRequest[k].first.getSocketFd())
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    std::cout << "Richiesta ricevuta" << std::endl;
                                    bool found = false;
                                    Response response;
                                    FD_CLR(fd, &fdreads[i]);
...
// void IOMultiplexing::EventLoop(std::vector<Server> &servers) //potrebbe essere utile creare una copia di fdread ed fdwrite
// {
// 	std::vector<int> epoll_fds(servers.size());
//     std::vector<fd_set> fdreads(servers.size());
//     std::vector<fd_set> fdwrites(servers.size());
...
// 				{
// 					int fd_server = servers[j].getSocket().getSocketFd();
// 					if (fd == fd_server)
// 					{
// 						struct sockaddr_in client_addr;
// 						socklen_t client_len = sizeof(client_addr);
// 						int client_fd = accept(fd_server, (struct sockaddr *)&client_addr, &client_len);
// 						if (client_fd == -1)
// 						{
// 							std::cerr << "Errore nella chiamata a accept" << std::endl;
// 							return;
// 						}
// 						fcntl(client_fd, F_SETFL, O_NONBLOCK);
// 						events[j].data()->events = EPOLLIN | EPOLLET;
// 						events[j].data()->data.fd = client_fd;
// 						if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, client_fd, events[j].data()) == -1)
// 						{
// 							std::cerr << "Errore nella chiamata a epoll_ctl" << std::endl;
// 							return;
// 						}
// 						Client client;
// 						client.setSocketFd(client_fd);
// 						client.setServer(servers[j]);
// 						Request request;
// 						ClientsRequest.push_back(std::pair<Client,Request>(client,request));
// 						std::cout << "Nuova connessione accettata sul server " << j << std::endl;
// 						FD_SET(client_fd, &fdread);
// 						if (client_fd > this->_fdmax)
// 						{
// 							this->_fdmax = client_fd;
// 						}
// 							std::string res="HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length: 12\r\n\r\nHello world!";
// 							send(client_fd, res.c_str(), res.length(), 0);
...
// 			for (size_t j = 0; j < ClientsRequest.size(); i++)
// 			{
// 				if (FD_ISSET(ClientsRequest[j].first.getSocketFd(), &fdread_cp))
// 				{
// 					char buffer[10002];
// 					int nread = recv(fd, buffer, 100000, 0);
// 					if (nread == -1)
// 					{
// 						FD_CLR(ClientsRequest[j].first.getSocketFd(), &fdread);
// 						close(ClientsRequest[j].first.getSocketFd());
// 						ClientsRequest.erase(ClientsRequest.begin() + j);
// 					}
// 					else if (nread == 0)
// 					{
// 						FD_CLR(ClientsRequest[j].first.getSocketFd(), &fdread);
...
// 					else
// 					{
// 						if (ClientsRequest[j].second.getOk() == 0)
// 							ClientsRequest[j].second = Request();
// 						ClientsRequest[j].second.setLength(nread);
// 						ClientsRequest[j].second.handleRequest(buffer);
// 						if (ClientsRequest[j].second.getFinished() ==1)
// 						{
// 							std::cout << "Richiesta ricevuta" << std::endl;
// 							bool found = false;
// 							Response response;
// 							FD_CLR(ClientsRequest[j].first.getSocketFd(), &fdread);
// 							FD_SET(ClientsRequest[j].first.getSocketFd(), &fdwrite);
// 							for (size_t j = 0; j < servers.size(); j++)
// 							{
// 								for (size_t x = 0; x < servers[j].getServerNames().size(); x++)
// 								{
// 									if (servers[j].getServerNames()[x] == strtrim(ClientsRequest[j].second.GetRequest().at("Host")))
// 									{
// 										response = Response(ClientsRequest[j].second, servers[j], ClientsRequest[j].first.getSocketFd());
// 										ResponseReady.push_back(response);
// 										found = true;
// 										// break;
// 									}
...
// 								for (size_t j = 0; j < servers.size(); j++)
// 								{
// 									if (!servers[j].getServerNames().size() && servers[j].getPort() == ClientsRequest[j].first.getServer().getPort())
// 									{
// 										response = Response(ClientsRequest[j].second, servers[j], ClientsRequest[j].first.getSocketFd());
// 										ResponseReady.push_back(response);
// 										found = true;
...
// 							if (!found)
// 							{
// 								response = Response(ClientsRequest[j].second, servers[0], ClientsRequest[j].first.getSocketFd());
// 								ResponseReady.push_back(response);
// 							}
// 						}
// 					}
// 				}
// 			}
// 			for (u_int i = 0; i < ResponseReady.size(); i++)
// 			{
// 				if(FD_ISSET(ResponseReady[i].getClientFD(), &fdwrite))
// 				{
// 				// 	ResponseReady[i].handler(fdread, fdwrite);
// 					if (ResponseReady[i].getDone()==1)
// 						ResponseReady.erase(ResponseReady.begin() + i);
// 				}
// 			}
